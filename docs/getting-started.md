...
Streamlit owns the top-level HTML document, so it has no template where the plain script tag below can be added. Scripts passed through `st.markdown(..., unsafe_allow_html=True)` are rendered but do not execute; `st.components.v1.html(...)` executes inside a component iframe instead.

Run `npx @reticlehq/server init` from the Streamlit project. It detects a `streamlit` dependency or import and prints a copy-pasteable `components.html` helper that installs one guarded module script in the parent app document. Keep that helper development-only and do not commit the pairing token printed inside it.
